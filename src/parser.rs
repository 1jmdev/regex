...
pub fn parse(pattern: &str) -> Result<Parsed, Error> {
    let mut p = Parser { chars: pattern.chars().collect(), pos: 0, captures: 0 };
    let ast = p.parse_alt()?;
    if p.peek().is_some() {
        return Err(Error::new("unexpected trailing input"));
    }
    Ok(Parsed { ast, captures: p.captures })
}
...
impl Parser {
    ...
    fn parse_alt(&mut self) -> Result<Ast, Error> {
        ...
        Ok(if parts.len() == 1 { parts.remove(0) } else { Ast::Alt(parts) })
    }
    ...
    fn parse_braces(&mut self) -> Result<(usize, Option<usize>), Error> {
        ...
        if !self.eat('}') { return Err(Error::new("unclosed repetition")); }
        if let Some(max) = max { if max < min { return Err(Error::new("invalid repetition range")); } }
        Ok((min, max))
    }
    ...
    fn number(&mut self) -> Result<usize, Error> {
        ...
        let mut n = 0usize;
        while let Some(c) = self.peek() {
            if let Some(d) = c.to_digit(10) { self.bump(); n = n.saturating_mul(10).saturating_add(d as usize); } else { break; }
        }
        if self.pos == start { Err(Error::new("expected number")) } else { Ok(n) }
    }

    fn parse_atom(&mut self) -> Result<Ast, Error> {
        match self.bump().ok_or_else(|| Error::new("unexpected end of pattern"))? {
            '.' => Ok(Ast::Dot),
            '^' => Ok(Ast::StartLine),
            '$' => Ok(Ast::EndLine),
            '(' => {
                if self.eat('?') { return Err(Error::new("unsupported group syntax")); }
                self.captures += 1;
                let index = self.captures;
                let node = self.parse_alt()?;
                if !self.eat(')') { return Err(Error::new("unclosed group")); }
    ...
    fn parse_escape(&mut self, in_class: bool) -> Result<Ast, Error> {
        ...
        Ok(match c {
            'd' => Ast::Class(Class { negated: false, items: vec![ClassItem::Digit] }),
            'D' => Ast::Class(Class { negated: true, items: vec![ClassItem::Digit] }),
            'w' => Ast::Class(Class { negated: false, items: vec![ClassItem::Word] }),
            'W' => Ast::Class(Class { negated: true, items: vec![ClassItem::Word] }),
            's' => Ast::Class(Class { negated: false, items: vec![ClassItem::Space] }),
            'S' => Ast::Class(Class { negated: true, items: vec![ClassItem::Space] }),
    ...
    fn parse_class(&mut self) -> Result<Ast, Error> {
        ...
        let mut first = true;
        while let Some(c) = self.peek() {
            if c == ']' && !first { self.bump(); return Ok(Ast::Class(Class { negated, items })); }
            first = false;
            ...
            if self.peek() == Some('-') {
                ...
                match (start, end) {
                    (ClassItem::Char(a), ClassItem::Char(b)) if a <= b => items.push(ClassItem::Range(a, b)),
    ...
    fn class_item(&mut self) -> Result<ClassItem, Error> {
        match self.bump().ok_or_else(|| Error::new("unclosed character class"))? {
